...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                # TASK 3: Add a slider to select payload rang
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(values=site_success.values, 
        names=site_success.index, 
        title='Success Launches by Launch Site')
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        site_success_count = filtered_df['class'].sum()
        site_total_launches = len(filtered_df)
        site_failure_count = site_total_launches - site_success_count

        fig = px.pie(values=[site_success_count, site_failure_count],
